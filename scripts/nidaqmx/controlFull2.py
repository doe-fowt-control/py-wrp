# synchronize three tasks
# 1 read task for wave gauges
# 1 read task for string pots
# 1 write task for MPC output``

import sys
sys.path.append('../py-wrp')

# gauges are configured in separate input file
from inputs2 import times, gauges, dm, wrp, stringPots
...
if __name__ == "__main__":
    ...
    with plt.ion(), nidaqmx.Task() as readTask, nidaqmx.Task() as writeTask, nidaqmx.Task() as floatTask:
...
# PORTS + TIMING
        ...
        for ai_channel in gauges.portNames:
            ...
            channel.ai_min = 0
            channel.ai_max = 5
            channel.ai_term_cfg = TerminalConfiguration.BAL_DIFF
        ...
        # float task
        for ai_channel in stringPots.portNames:
            channel = floatTask.ai_channels.add_ai_voltage_chan(ai_channel)
            channel.ai_min = 0
            channel.ai_max = 5
            channel.ai_term_cfg = TerminalConfiguration.BAL_DIFF
        
        floatTask.timing.cfg_samp_clk_timing(
            rate = dm.readRate * 2, 
            sample_mode = AcquisitionType.CONTINUOUS, 
            samps_per_chan = dm.read.nSamples * 2,
            source = 'OnboardClock',
        )
...
# writeTask trigger
        ...
        # floatTask trigger
        floatTask.triggers.start_trigger.cfg_dig_edge_start_trig(
            trigger_source=readTask.triggers.start_trigger.term,
        )
        ...
        def float_callback(task_handle, every_n_samples_event_type, number_of_samples, callback_data):
            floatReader.read_many_sample(
                dm.read.values,
    ...
    # STREAMS
        reader = AnalogMultiChannelReader( # multi channel
            readTask.in_stream)
        floatReader = AnalogMultiChannelReader(
            floatTask.in_stream)
