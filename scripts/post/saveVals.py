...
# files to read statically
    load = DataLoader(
        'data/irreg_eta_stephanie.csv',
        'data/irreg_t_stephanie.csv',
    ...
    r1 = 60
    while r0 < r1:
        load.generateBuffersStatic(dm, r0)
        wrp.spectral(dm)
        wrp.inversion(dm)
        wrp.reconstruct(dm)

        eta_measured.extend(np.squeeze(dm.predictValues))
        eta_predicted.extend(wrp.reconstructedSurfacePredict)
        
        new_aphio = np.transpose(np.hstack((wrp.A, wrp.phi, wrp.w)))
        aphio = np.hstack((aphio, new_aphio))

        r0 += dm.updateInterval
...
#     true_pred = true_pred[:, 2000:]
    ...
    np.savetxt("irreg_A_phi_omega.csv", aphio, delimiter=',')
    np.savetxt("irreg_eta_true_predicted.csv", true_pred, delimiter = ",")
