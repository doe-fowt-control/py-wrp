function dxdt = seakeepingContinuousTime(time, x, gravity, density_water, ...
    ...
    float_excitation_data, eta_full, time_full, fs)

eta = interp1(time_full, eta_full, time);

% dt = 1/fs;
% 
% et = 5;     % length of sample used for excitation frequencies
% time_sample = time : dt : time + et - dt;
% eta_sample = interp1(time_full, eta_full, time_sample);
% 
% eta = eta_sample(1);
...
[plant_amplitudes,...
    plant_phases,...
    plant_frequencies] = ...
    fft_decomp(fs, eta_full);


% excitation_time = time - floor(time/et)*et; % between zero and one second

[total_excitation_force_heave, total_excitation_force_roll] ...
    = excitationForceCalculator(float_excitation_data, ...
    time, plant_amplitudes', plant_frequencies, ...
    plant_phases');


% Measured disturbance inputs (fixed for this example)
mv = control_rack_length / 2;
...
h1 = total_excitation_force_heave;
h2 = viscous_force_heave;
h3 = radiative_damping_memory_force_heave;
h4 = restoring_force_heave;
h5 = restoring_force_heave_roll;
h6 = total_mass_heave;
